    ...
    <section className="py-24 sm:py-32 bg-gray-50">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
          {homeData.focusAreasSection.focusAreas.map((area) => (
            <div
              key={area._key}
              className="relative flex flex-col gap-6 bg-white p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all group"
            >
              <div className={`inline-flex h-14 w-14 items-center justify-center rounded-lg bg-red-700 group-hover:bg-red-500 text-white group-hover:scale-110 transition-transform`}>
                <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                  <path  d={area.icon} />
                </svg>
              </div>
